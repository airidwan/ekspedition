...
class InvoicePickupController extends Controller
{
    const RESOURCE = 'Accountreceivables\Transaction\InvoicePickup';
    const URL = 'accountreceivables/transaction/invoice-pickup';
    ...
    public function index(Request $request)
    {
        ...
        $query   = \DB::table('ar.inv_ar_header')
                        ...
                        ->where('inv_ar_header.type', '=', InvoiceArHeader::INV_PICKUP)
        ...
        return view('accountreceivables::transaction.invoice-pickup.index', [
    ...
    public function add(Request $request)
    {
        ...
        return view('accountreceivables::transaction.invoice-pickup.add', [
    ...
    public function save(Request $request)
    {
        ...
        if (empty($model->type)) {
            $model->type = InvoiceArHeader::INV_PICKUP;
        }
        ...
        $request->session()->flash(
            'successMessage',
            trans('shared/common.saved-message', ['variable' => trans('accountreceivables/menu.invoice-pickup').' '.$model->inv_ar_number])
        );
    ...
    public function getJsonResi(Request $request)
    {
        ...
        $query = \DB::table('op.trans_resi_header')
                    ->select(
                        'trans_resi_header.*', 'trans_pickup_request.pickup_request_id', 'trans_pickup_request.pickup_request_number', 'trans_pickup_request.pickup_cost',
                        'mst_route.route_code', 'mst_customer.customer_name', 'customer_receiver.customer_name as customer_receiver_name'
                    )
                    ->join('mrk.trans_pickup_request', 'trans_resi_header.pickup_request_id', '=', 'trans_pickup_request.pickup_request_id')
                    ...
                    ->where('trans_pickup_request.pickup_cost', '<>', 0)
