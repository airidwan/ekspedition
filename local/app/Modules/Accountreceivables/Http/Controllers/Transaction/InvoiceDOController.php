...
{
    const RESOURCE = 'Accountreceivables\Transaction\InvoiceDO';
    const URL = 'accountreceivables/transaction/invoice-do';
    ...
    public function index(Request $request)
    {
        ...
        $query   = \DB::table('ar.inv_ar_header')
                        ...
                        ->where('inv_ar_header.type', '=', InvoiceArHeader::INV_DO)
        ...
        return view('accountreceivables::transaction.invoice-do.index', [
    ...
    public function add(Request $request)
    {
        ...
        return view('accountreceivables::transaction.invoice-do.add', [
    ...
    public function save(Request $request)
    {
        ...
        if (empty($model->type)) {
            $model->type = InvoiceArHeader::INV_DO;
        }
        ...
        if ($model->isOpen()) {
            ...
            for ($i=0; $i < count($request->get('lineId')); $i++) { 
                ...
                $line->delivery_order_line_id = $request->get('doLineId')[$i];
        ...
        $request->session()->flash(
            'successMessage',
            trans('shared/common.saved-message', ['variable' => trans('accountreceivables/menu.invoice-do').' '.$model->inv_ar_number])
        );
    ...
    public function getJsonResi(Request $request)
    {
        ...
        $query = \DB::table('op.trans_resi_header')
                    ->select(
                        'trans_resi_header.*', 'trans_delivery_order_line.delivery_order_line_id', 'trans_delivery_order_header.delivery_order_number', 'trans_delivery_order_line.delivery_cost',
                        'mst_route.route_code', 'mst_customer.customer_name', 'customer_receiver.customer_name as customer_receiver_name'
                    )
                    ->join('op.trans_delivery_order_line', 'trans_resi_header.resi_header_id', '=', 'trans_delivery_order_line.resi_header_id')
                    ->join('op.trans_delivery_order_header', 'trans_delivery_order_line.delivery_order_header_id', '=', 'trans_delivery_order_header.delivery_order_header_id')
                    ...
                    ->where('trans_delivery_order_line.delivery_cost', '<>', 0)
        ...
        foreach ($query->get() as $resi) {
            ...
            $resi->total_weight = $modelResi->totalWeight();
            $resi->total_volume = $modelResi->totalVolume();
            $resi->total_unit   = $modelResi->totalUnit();
