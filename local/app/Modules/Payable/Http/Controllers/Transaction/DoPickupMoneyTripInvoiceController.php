...
class DoPickupMoneyTripInvoiceController extends Controller
{
    const RESOURCE = 'Payable\Transaction\DoPickupMoneyTripInvoice';
    const URL      = 'payable/transaction/do-pickup-money-trip-invoice';
    ...
    public function index(Request $request)
    {
        ...
        $query   = \DB::table('ap.invoice_header')
                    ...
                    ->leftJoin('op.trans_delivery_order_header', 'trans_delivery_order_header.delivery_order_header_id', '=', 'invoice_line.do_header_id')
                    ->leftJoin('op.trans_pickup_form_header', 'trans_pickup_form_header.pickup_form_header_id', '=', 'invoice_line.pickup_form_header_id')
                    ->join('ap.mst_ap_type', 'mst_ap_type.type_id', '=', 'invoice_header.type_id')
                    ->where(function ($query) {
                            $query->where('invoice_header.type_id', '=', InvoiceHeader::DO_MONEY_TRIP)
                                  ->orWhere('invoice_header.type_id', '=', InvoiceHeader::PICKUP_MONEY_TRIP);
                        })
        ...
        if (!empty($filters['doPickupNumber'])) {
            $query->where(function ($query) use ($filters){
                        $query->where('trans_delivery_order_header.delivery_order_number', 'ilike', '%'.$filters['doPickupNumber'].'%')
                              ->orWhere('trans_pickup_form_header.pickup_form_number', 'ilike', '%'.$filters['doPickupNumber'].'%');
                    });
        }
        ...
        return view('payable::transaction.do-pickup-money-trip-invoice.index', [
    ...
    public function add(Request $request)
    {
        ...
        return view('payable::transaction.do-pickup-money-trip-invoice.add', [
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($request->user()->can('access', [self::RESOURCE, 'update'])) {
            return view('payable::transaction.do-pickup-money-trip-invoice.add', $data);
        } else {
            return view('payable::transaction.do-pickup-money-trip-invoice.detail', $data);
        }
    }
    ...
    public function save(Request $request)
    {
        ...
        if ($request->get('type') == InvoiceHeader::DO_MONEY_TRIP) {
             $this->validate($request, [
                'doId' => 'required',
            ]);
        }else{
        ...
        if (!empty($request->get('pickupId'))) {
            $modelLine->pickup_form_header_id = $request->get('pickupId');
        }elseif(!empty($request->get('doId'))){
            $modelLine->do_header_id = $request->get('doId');
        }
        ...
        if ($request->get('btn-approve') !== null) {
            ...
            if ($modelPayableType->type_id == InvoiceHeader::DO_MONEY_TRIP) {
                $modelDoHeader = DeliveryOrderHeader::find($modelLine->do_header_id);
                $note          = $model->invoice_number.' - '.$modelDoHeader->delivery_order_number; 
            }else{
                $modelPickupHeader = PickupFormHeader::find($modelLine->pickup_form_header_id);
                $note              = $model->invoice_number.' - '.$modelPickupHeader->pickup_form_number; 
            }

            // insert journal
            $journalHeader           = new JournalHeader();
            $journalHeader->category = JournalHeader::INVOICE_DO_PICKUP_MONEY_TRIP;
            ...
            $journalHeader->description = $note;
            ...
            foreach ($userNotif as $user) {
                ...
                $notif->category   = 'Invoice DO/Pickup Money Trip Approved';
                $notif->message    = 'Invoice DO/Pickup Money Trip - '.$model->invoice_number.' is ready to be paid.';
        ...
        $request->session()->flash(
            'successMessage',
            trans('shared/common.saved-message', ['variable' => trans('payable/menu.do-pickup-money-trip-invoice').' '.$model->invoice_number])
        );
    ...
    public function cancel(Request $request)
    {
        ...
        if ($model->getTotalPayment() > 0) {
            return redirect(\URL::previous())->withInput($request->all())->withErrors(['errorMessage' => trans('payable/menu.do-pickup-money-trip-invoice').' '.$model->invoice_number.' '.trans('shared/common.cannot-cancel-payment-exist'). '. Payment exist on '.number_format($model->getTotalPayment()) ]);
        }
        ...
        if ($model->status == InvoiceHeader::APPROVED) {
            ...
            // insert journal
            $journalHeader           = new JournalHeader();
            $journalHeader->category = JournalHeader::CANCEL_INVOICE_DO_PICKUP_MONEY_TRIP;
            $journalHeader->status   = JournalHeader::OPEN;
        ...
        foreach ($userNotif as $user) {
            $notif             = new Notification();
            $notif->branch_id  = \Session::get('currentBranch')->branch_id;
            $notif->category   = 'Invoice DO/Pickup Money Trip Canceled';
            $notif->message    = 'Invoice DO/Pickup Money Trip Canceled '.$model->invoice_number.' Canceled reason is "'.$request->get('reason').'"'; 
        ...
        $request->session()->flash(
            'successMessage',
            trans('purchasing/fields.canceled-message', ['variable' => trans('payable/menu.do-pickup-money-trip-invoice').' '.$model->invoice_number])
        );
    ...
    public function printPdfDetail(Request $request, $id)
    {
        ...
        $header = view('print.header-pdf', ['title' => trans('payable/menu.do-pickup-money-trip-invoice')])->render();
        \PDF::setHeaderCallback(function($pdf) use ($header) {
            $pdf->writeHTML($header);
        });

        $html = view('payable::transaction.do-pickup-money-trip-invoice.print-pdf-detail', [
            'model'  => $model,
            'filters' => $filters,
        ])->render();

        \PDF::SetTitle(trans('payable/menu.do-pickup-money-trip-invoice'));
    ...
    public function getJsonDriver(Request $request)
    {
        ...
        $query->select('mst_driver.driver_id', 'mst_driver.driver_code', 'mst_driver.driver_nickname', 'mst_driver.driver_name', 'mst_driver.address', 'mst_driver.phone_number', 'mst_lookup_values.meaning as position')
    ...
    public function getJsonDo(Request $request)
    {
        ...
        $query = DeliveryOrderService::getQueryDeliveryOrderMinimumApproved();

        $query->select(
                    'trans_delivery_order_header.*', 
                    'driver.driver_code', 
                    'driver.driver_name', 
                    ...
                    'mst_truck.type as truck_type'
                    )
                ->orderBy('trans_delivery_order_header.delivery_order_number', 'desc')
                ->where('trans_delivery_order_header.driver_id', '=', $driverId)
                ->whereRaw('trans_delivery_order_header.delivery_order_header_id not in (select invoice_line.do_header_id from ap.invoice_line join ap.invoice_header on invoice_header.header_id = invoice_line.header_id where invoice_header.status <> \''.InvoiceHeader::CANCELED .'\' and invoice_line.do_header_id is not null)')
                ->where(function ($query) use ($search) {
        ...
        foreach ($query->get() as $model) {
            $moneyTrip  = \DB::table('op.mst_delivery_area_money_trip')
                                ->where('delivery_area_id', '=', $model->delivery_area_id)
                                ->where('vehicle_type', '=', $model->truck_type)
                                ->where('branch_id', '=', \Session::get('currentBranch')->branch_id)
                                ->first();
            if (!empty($moneyTrip)) {
                $model->money_trip = $moneyTrip->money_trip_rate;
            }else{
                $model->money_trip = 0;
            }
            $arrModel[] = $model;
    ...
    public function getJsonPickup(Request $request)
    {
        $search   = $request->get('search');
        $driverId = $request->get('driverId');
        $query    = PickupService::getQueryPickupClosed();

        $query->select(
                    'trans_pickup_form_header.pickup_form_header_id', 
                    'trans_pickup_form_header.pickup_form_number', 
                    'trans_pickup_form_header.created_date', 
                    'trans_pickup_form_header.note', 
                    'mst_truck.police_number', 
                    'mst_driver.driver_code', 
                    'mst_driver.driver_name'
                    )
                ->orderBy('trans_pickup_form_header.pickup_form_number', 'desc')
                ->where('trans_pickup_form_header.driver_id', '=', $driverId)
                ->whereRaw('trans_pickup_form_header.pickup_form_header_id not in (select pickup_form_header_id from ap.invoice_line join ap.invoice_header on invoice_header.header_id = invoice_line.header_id where invoice_header.status <> \''.InvoiceHeader::CANCELED .'\' and invoice_line.pickup_form_header_id is not null)')
                ->where(function ($query) use ($search) {
                    $query->where('trans_pickup_form_header.pickup_form_number', 'ilike', '%'.$search.'%')
                          ->orWhere('mst_driver.driver_name', 'ilike', '%'.$search.'%')
                          ->orWhere('trans_pickup_form_header.police_number', 'ilike', '%'.$search.'%')
    ...
    public function optionType()
    {
        return \DB::table('ap.mst_ap_type')
                    ->where(function ($query) {
                            $query->where('mst_ap_type.type_id', '=', InvoiceHeader::DO_MONEY_TRIP)
                                  ->orWhere('mst_ap_type.type_id', '=', InvoiceHeader::PICKUP_MONEY_TRIP);
                        })
                    ->get();
    }
