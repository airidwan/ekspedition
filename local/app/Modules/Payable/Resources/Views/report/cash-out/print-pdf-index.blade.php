        ...
        <td width="75%" cellpadding="0" cellspacing="0">
            <table>
                @if (!empty($filters['branchId']))
                    <?php $branch = \DB::table('op.mst_branch')->select('branch_name')->where('branch_id', '=', $filters['branchId'])->first(); ?>
                    <tr>
                        <td width="18%">{{ trans('shared/common.branch') }}</td>
                        <td width="2%">:</td>
...
<table class="table" cellspacing="0" cellpadding="2" border="1">
    ...
    <tbody>
        ...
        <tr>
            <td width="3%" rowspan="2" align="center">{{ $no++ }}</td>
            <td width="10%" >{{ $model->journal_number }}</td>
            <td width="10%" ></td>
            <td width="15%" ></td>
            <td width="18%" rowspan="2"></td>
            <td width="18%" rowspan="2">{{ $model->description }}</td>
            <td width="10%" >{{ $model->coa_code }}</td>
            <td width="8%" ></td>
            <td width="8%" rowspan="2" align="right">{{ number_format($model->credit) }}</td>
        </tr>
        <tr>
            <td></td>
            <td>{{ $date->format('d-m-Y') }}</td>
