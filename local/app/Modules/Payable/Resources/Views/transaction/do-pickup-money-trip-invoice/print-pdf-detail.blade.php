...
<table class="table" cellspacing="0" cellpadding="2" border="0">
    <thead>
        <tr>
            ...
            <th width="30%" style="border: 1px solid black">{{ trans('operational/fields.do-number').'/'.trans('operational/fields.pickup-number') }}</th>
            <th width="20%" style="border: 1px solid black">{{ trans('shared/common.description') }}</th>
            <th width="15%" style="border: 1px solid black">{{ trans('payable/fields.amount') }}</th>
            <th width="15%" style="border: 1px solid black">{{ trans('payable/fields.tax') }}</th>
            <th width="15%" style="border: 1px solid black">{{ trans('payable/fields.total-amount') }}</th>
        </tr>
    </thead>
    <tbody>
         <?php $no = 1; ?>
         ...
         <?php  
            ...
            if ($model->type_id == InvoiceHeader::DO_MONEY_TRIP) {
                $doPickup = $modelLine->deliveryOrder;
                $doPickupNumber = $doPickup->delivery_order_number;
                $doPickupDesc   = $doPickup->note;
            }else{
                $doPickup = $modelLine->pickup;
                $doPickupNumber = $doPickup->pickup_form_number;
                $doPickupDesc = $doPickup->note;
            }
        ?>
        <tr>
            <td width="5%" align="center" style="border: 1px solid black">{{ $no++ }}</td>
            <td width="30%" style="border: 1px solid black">{{ $doPickupNumber }}</td>
            <td width="20%" style="border: 1px solid black">{{ $doPickupDesc }}</td>
            <td width="15%" align="right" style="border: 1px solid black">{{ number_format($line->amount) }}</td>
            <td width="15%" align="right" style="border: 1px solid black">{{ number_format($modelLine->totalTax()) }}</td>
            <td width="15%" align="right" style="border: 1px solid black">{{ number_format($modelLine->totalAmount()) }}</td>
        </tr>
        @endforeach
        <tr>
            <td width="70%" align="right"> </td>
            <td width="15%" align="right" style="border: 1px solid black">{{ trans('shared/common.total-invoice') }} </td>
            <td width="15%" align="right" style="border: 1px solid black">{{ number_format($model->getTotalInvoice()) }}</td>
        </tr>
        <tr>
            <td width="70%" align="right"> </td>
            <td width="15%" align="right" style="border: 1px solid black">{{ trans('shared/common.total-payment') }} </td>
            <td width="15%" align="right" style="border: 1px solid black">{{ number_format($model->getTotalPayment()) }}</td>
        </tr>
        <tr>
            <td width="70%" align="right"> </td>
            <td width="15%" align="right" style="border: 1px solid black"><strong>{{ trans('shared/common.total-remain') }} </strong></td>
            <td width="15%" align="right" style="border: 1px solid black"><strong>{{ number_format($model->getTotalRemain()) }}</strong></td>
        </tr>
    </tbody>
</table>
