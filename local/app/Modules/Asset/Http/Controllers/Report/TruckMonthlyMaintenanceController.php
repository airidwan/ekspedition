...
class TruckMonthlyMaintenanceController extends Controller
{
    const RESOURCE = 'Asset\Report\TruckMonthlyMaintenance';
    const URL      = 'asset/report/truck-monthly-maintenance';
    protected $now;
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($filters['policeNumber']) || !empty($filters['ownerName']) || !empty($filters['policeNumber']) || !empty($filters['employee']) || !empty($filters['category']) || !empty($filters['status']) || !empty($filters['dateTo']) || !empty($filters['dateFrom'])) {
        ...
        return view('asset::report.truck-monthly-maintenance.index', [
    ...
    protected function getQueryMoveOrder(Request $request, $filters){
        ...
        if (!empty($filters['ownerName'])) {
            $queryMoveOrder->where('mst_truck.owner_name', 'ilike', '%'.$filters['ownerName'].'%');
        }
    ...
    protected function getQueryPurchaseOrder(Request $request, $filters){
        ...
        if (!empty($filters['ownerName'])) {
            $queryPurchaseOrder->where('mst_truck.owner_name', 'ilike', '%'.$filters['ownerName'].'%');
        }
    ...
    protected function getQueryInvoice(Request $request, $filters){
        ...
        if (!empty($filters['ownerName'])) {
            $queryInvoice->where('mst_truck.owner_name', 'ilike', '%'.$filters['ownerName'].'%');
        }
    ...
    public function printPdf(Request $request)
    {
        ...
        if (!empty($filters['policeNumber']) || !empty($filters['ownerName']) || !empty($filters['status']) || !empty($filters['dateTo']) || !empty($filters['dateFrom'])) {
            $queryPurchaseOrder = $this->getQueryPurchaseOrder($request, $filters);
            $queryMoveOrder     = $this->getQueryMoveOrder($request, $filters);
            $queryInvoice       = $this->getQueryInvoice($request, $filters);
        }

        $header = view('print.header-pdf', ['title' => trans('asset/menu.truck-monthly-maintenance')])->render();
        \PDF::setHeaderCallback(function($pdf) use ($header) {
            $pdf->writeHTML($header);
        });

        $html = view('asset::report.truck-monthly-maintenance.print-pdf', [
            'purchaseOrder' => !empty($queryPurchaseOrder) ? $queryPurchaseOrder : [],
            'moveOrder'     => !empty($queryMoveOrder) ? $queryMoveOrder : [],
            'invoice'       => !empty($queryInvoice) ? $queryInvoice : [],
            'filters'       => $filters,
        ...
        \PDF::SetTitle(trans('asset/menu.truck-monthly-maintenance'));
        \PDF::SetMargins(5, 20, 5, 0);
        \PDF::SetAutoPageBreak(TRUE, 10);
        \PDF::AddPage('L', 'A4');
        \PDF::writeHTML($html);
        \PDF::Output(trans('asset/menu.truck-monthly-maintenance').' '.\Session::get('currentBranch')->branch_code.'.pdf');
        \PDF::reset();
    }
    ...
    public function printExcel(Request $request)
    {
        ...
        \Excel::create(trans('asset/menu.truck-monthly-maintenance'), function($excel) use ($queryPurchaseOrder, $queryMoveOrder, $queryInvoice, $filters) {
            $excel->sheet('Sheet1', function($sheet) use ($queryPurchaseOrder, $queryMoveOrder, $queryInvoice, $filters) {
                $sheet->cell('A1', function($cell) {
                    ...
                    $cell->setValue(trans('asset/menu.truck-monthly-maintenance'));
                });
