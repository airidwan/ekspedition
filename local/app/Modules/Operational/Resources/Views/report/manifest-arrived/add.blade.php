    ...
    <div class="col-md-12">
        <div class="widget">
            <div class="widget-header">
                <h2><i class="fa fa-truck"></i> <strong>{{ $title }}</strong> {{ trans('operational/menu.manifest-arrived') }}</h2>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade active in" id="tabHeaders">
                                <div class="col-sm-6 portlets">
                                    ...
                                    <div class="form-group">
                                        <label for="arrivedDate" class="col-sm-4 control-label">{{ trans('operational/fields.arrived-date') }}</label>
                                        <div class="col-sm-8">
                                            <div class="input-group">
                                                <?php $arrivedDate = !empty($model->arrive_date) ? new \DateTime($model->arrive_date) : null; ?>
                                                <input type="text" id="arrivedDate" name="arrivedDate" class="form-control datepicker-input" data-mask="99-99-9999" placeholder="dd-mm-yyyy" value="{{ $arrivedDate !== null ? $arrivedDate->format('d-m-Y') : '' }}" disabled/>
                                                <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="col-sm-12 portlets">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered" id="table-line" cellspacing="0" width="100%">
                                            <thead>
                                                <tr>
                                                    <th>{{ trans('operational/fields.resi-number') }}<hr/>{{ trans('operational/fields.resi-date') }}</th>
                                                    ...
                                                    <th>{{ trans('operational/fields.total-coly') }}<hr/>{{ trans('operational/fields.coly-sent') }}</th>
                                                    <th>{{ trans('operational/fields.coly-receipt') }}<hr/>{{ trans('operational/fields.coly-wh') }}</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <?php
                                                ...
                                                $customerReceiver = $line->resi !== null ? $line->resi->customerReceiver : null;
                                                $route = $line->resi !== null ? $line->resi->route : null;
                                                $resiReceipt = 0;
                                                foreach ($line->receiptManifestLines as $receipt) {
                                                     $resiReceipt += $receipt->coly_receipt;
                                                } 
                                                $resiStock  = \DB::table('op.mst_stock_resi')->where('resi_header_id', '=', $line->resi_header_id)->where('branch_id', '=', \Session::get('currentBranch')->branch_id)->first();
                                                $itemName = '';
                                                if ($line->resi !== null) {
                                                ...
                                                ?>
                                                <tr>
                                                    <td > {{ $line->resi !== null ? $line->resi->resi_number : '' }} <hr/> {{ $resiDate !== null ? $resiDate->format('d-m-Y') : '' }} </td>
                                                    <td > {{ $route !== null ? $route->route_code : '' }} </td>
                                                    <td > {{ $customer !== null ? $customer->customer_name : '' }} <hr/> {{ $line->resi !== null ? $line->resi->sender_name : '' }}</td>
                                                    <td > {{ $customerReceiver !== null ? $customerReceiver->customer_name : '' }} <hr/> {{ $line->resi !== null ? $line->resi->receiver_name : '' }}</td>
                                                    <td > {{ $itemName }} </td>
                                                    <td class="text-right"> {{ number_format($line->resi !== null ? $line->resi->totalColy() : 0) }} <hr/> {{ number_format($line->coly_sent) }} </td>
                                                    <td class="text-right"> {{ number_format($resiReceipt) }} <hr/> {{ number_format(!empty($resiStock) ? $resiStock->coly : 0) }} </td>
                                                </tr>
