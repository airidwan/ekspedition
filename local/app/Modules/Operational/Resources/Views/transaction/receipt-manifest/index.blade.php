    ...
    <div class="col-md-12">
        <div class="widget">
            <div class="widget-header">
                <h2><i class="fa fa-truck"></i> {{ trans('operational/menu.receipt-manifest') }}</h2>
                    ...
                    <form  role="form" id="add-form" class="form-horizontal" method="post" action="">
                        ...
                        <div class="col-sm-4 portlets">
                            <div class="form-group">
                                <label for="receiptManifestNumber" class="col-sm-4 control-label">{{ trans('inventory/fields.receipt-number') }}</label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="receiptManifestNumber" name="receiptManifestNumber" value="{{ !empty($filters['receiptManifestNumber']) ? $filters['receiptManifestNumber'] : '' }}">
                                </div>
                            </div>
                            <div class="form-group">
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-striped table-bordered" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            ...
                            <?php
                                $receiptDate = !empty($model->created_date) ? \App\Service\TimezoneDateConverter::getClientDateTime($model->created_date) : null;
                            ?>
                            <tr>
                                <td class="text-center">{{ $no++ }}</td>
                                <td>{{ $model->manifest_receipt_number }}</td>
                                <td>{{ !empty($receiptDate) ? $receiptDate->format('d-M-Y') : '' }}</td>
                                <td>{{ !empty($receiptDate) ? $receiptDate->format('h:i') : '' }}</td>
                                <td>{{ $model->manifest_number }}</td>
                                <td>{{ $model->note }}</td>
                                <td class="text-center">
                                @can('access', [$resource, 'update'])
                                    <a href="{{ URL($url . '/edit/' . $model->manifest_receipt_header_id) }}" data-toggle="tooltip" class="btn btn-xs btn-warning" data-original-title="{{ trans('shared/common.edit') }}">
                    ...
                    <table class="table table-striped table-bordered" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th>{{ trans('operational/fields.coly-receipt') }}</th>
                                <th>{{ trans('shared/common.action') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">{{ $model->coly_receipt }}</td>
                                <td class="text-center">
                                @can('access', [$resource, 'update'])
                                    <a href="{{ URL($url . '/edit/' . $model->manifest_receipt_header_id) }}" data-toggle="tooltip" class="btn btn-xs btn-warning" data-original-title="{{ trans('shared/common.edit') }}">
                                    <i class="fa fa-pencil"></i>
