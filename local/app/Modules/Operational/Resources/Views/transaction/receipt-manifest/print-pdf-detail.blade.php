...
<table id="filtes" width="100%" cellpadding="0" cellspacing="0">
    <tr>
        <td width="33%" cellpadding="0" cellspacing="0">
            <table>
                <tr>
                    <td width="38%">{{ trans('operational/fields.receipt-number') }}</td>
                    <td width="2%">:</td>
                    <td width="60%">{{ $model->manifest_receipt_number }}</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td width="2%">:</td>
                    <td width="60%">{{ !empty($date) ? $date->format('d-m-Y') : '' }}</td>
                </tr>
            </table>
            <br/>
        </td>
...
<table class="table" cellspacing="0" cellpadding="2" border="0">
    <thead>
        <tr>
            ...
            <th width="25%" style="border: 1px solid black" rowspan="2">{{ trans('operational/fields.item-name') }}</th>
            <th width="15%" style="border: 1px solid black" rowspan="2">{{ trans('operational/fields.receiver') }}</th>
            <th width="6%" style="border: 1px solid black">{{ trans('operational/fields.total-coly') }}</th>
            <th width="6%" style="border: 1px solid black" rowspan="2">{{ trans('operational/fields.coly-receipt') }}</th>
            <th width="8%" style="border: 1px solid black">{{ trans('operational/fields.weight') }}</th>
        ...
        <tr>
            <th style="border: 1px solid black">{{ trans('operational/fields.route-code') }}</th>
    ...
    <tbody>
     ...
     <tr>
        ...
        <td width="15%" style="border: 1px solid black">{{ $line->manifestLine->resi->resi_number }}</td>
        <td width="25%" style="border: 1px solid black" rowspan="2">{{ $line->manifestLine->resi->item_name }}</td>
        <td width="15%" style="border: 1px solid black" rowspan="2">{{ $line->manifestLine->resi->receiver_name }}</td>
        <td width="6%" align="right" style="border: 1px solid black">{{ number_format($line->manifestLine->resi->totalColy()) }}</td>
        <td width="6%" align="right" style="border: 1px solid black" rowspan="2">{{ number_format($line->coly_receipt) }}</td>
        <td width="8%" align="right" style="border: 1px solid black">{{ number_format($line->manifestLine->resi->totalWeightAll()) }}</td>
    ...
    <tr>
        <td style="border: 1px solid black">{{ $line->manifestLine->resi->route->route_code }}</td>
