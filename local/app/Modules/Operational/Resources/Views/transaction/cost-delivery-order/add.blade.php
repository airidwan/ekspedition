...
use App\Modules\Operational\Http\Controllers\Transaction\DeliveryOrderController;
use App\Service\TimezoneDateConverter;
?>

@extends('layouts.master')

@section('title', trans('operational/menu.cost-delivery-order'))
    ...
    <div class="col-md-12">
        <div class="widget">
            <div class="widget-header">
                <h2><i class="fa fa-truck"></i> <strong>{{ $title }}</strong> {{ trans('operational/menu.cost-delivery-order') }}</h2>
                    ...
                    <form  role="form" id="add-form" class="form-horizontal" method="post" action="{{ url($url . '/save') }}">
                                        ...
                                        <table class="table table-striped table-bordered" id="table-line" cellspacing="0" width="100%">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>{{ trans('accountreceivables/fields.extra-cost') }}</th>
                                            ...
                                            <tbody>
                                                ...
                                                <?php
                                                    $resi     = $line->resi;
                                                    $customer = !empty($resi) ? $resi->customerReceiver : null;
                                                ?>
                                                <tr data-index="{{ $dataIndex }}">
                                                    ...
                                                    <td class="text-right">
                                                        ...
                                                        {{ $resi !== null ? number_format($resi->totalReceipt()) : '' }}
                                                    </td>
                                                    <td class="text-right"> {{ $line !== null ? number_format($line->total_coly) : '' }} </td>
                                                    <td>
                                                    @foreach($resi->invoices as $invoice)
                                                        @if($invoice->isInvoiceExtraCost())
                                                            {{ $invoice->invoice_number }} ({{ $invoice->description }}): {{ number_format($invoice->amount) }} <br/>
                                                        @endif
                                                    @endforeach
                                                    </td>
                                                    <td class="text-right">
                                                        <input type="text" class="form-control currency" name="deliveryCost[]" value="{{ count($errors) > 0 ? str_replace(',', '', old('deliveryCost')[$dataIndex]) : $line->delivery_cost }}" {{ $model->isConfirmed() ? 'readonly' : '' }}>
                        ...
                        <div class="col-sm-12 data-table-toolbar text-right">
                            <div class="form-group">
                                ...
                                @if(Gate::check('access', [$resource, 'reject']) && $model->status == DeliveryOrderHeader::APPROVED)
                                    <button type="submit" name="btn-reject" class="btn btn-sm btn-danger">
                                        <i class="fa fa-remove"></i> {{ trans('shared/common.reject') }}
                                    </button>
                                @endif

                                @if($model->status == DeliveryOrderHeader::APPROVED)
                                    <button type="submit" name="btn-confirm" class="btn btn-sm btn-info">
                                        <i class="fa fa-money"></i> {{ trans('operational/fields.confirm-cost') }}
                                    </button>
                                @endif

                                @if(Gate::check('access', [$resource, 'otr']) && $model->status == DeliveryOrderHeader::CONFIRMED)
                                    <button type="submit" name="btn-otr" class="btn btn-sm btn-info">
                                        <i class="fa fa-truck"></i> {{ trans('operational/fields.otr') }}
                                    </button>
                                @endif

                                @if($model->status == DeliveryOrderHeader::CONFIRMED)
                                    <a href="{{ URL(DeliveryOrderController::URL.'/print-pdf-detail/'.$model->delivery_order_header_id) }}" class="button btn btn-sm btn-success" target="_blank">
