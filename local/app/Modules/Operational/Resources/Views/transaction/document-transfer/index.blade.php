...
<?php use App\Modules\Operational\Model\Transaction\DocumentTransferHeader; ?>
    ...
    <div class="col-md-12">
        <div class="widget">
            <div class="widget-header">
                <h2><i class="fa fa-briefcase"></i> {{ trans('operational/menu.document-transfer') }}</h2>
                ...
                <div id="horizontal-form">
                    <form  role="form" id="add-form" class="form-horizontal" method="post" action="">
                        ...
                        <div class="col-sm-6 portlets">
                            ...
                            <div class="form-group">
                                <label for="toCity" class="col-sm-4 control-label">{{ trans('operational/fields.to-city') }}</label>
                                <div class="col-sm-8">
                                    <select class="form-control" id="toCity" name="toCity">
                                        <option value="" >ALL</option>
                                        @foreach($optionRoute as $toCity)
                                        <option value="{{ $toCity->city_end_id }}" {{ !empty($filters['toCity']) && $filters['toCity'] == $toCity->city_end_id ? 'selected' : '' }}>{{ $toCity->route_code }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <table class="table table-striped table-bordered" cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th>{{ trans('operational/fields.to-city') }}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <a href="{{ URL($url . '/edit/' . $model->document_transfer_header_id) }}" data-toggle="tooltip" class="btn btn-xs btn-warning" data-original-title="{{ trans('shared/common.edit') }}">
                                        <i class="fa fa-pencil"></i>
                                    </a>
                                    @endcan
                                    @if(Gate::check('access', [$resource, 'cancel']) && $model->status == DocumentTransferHeader::INCOMPLETE)
                                    <a data-id="{{ $model->document_transfer_header_id }}" data-label="{{ $model->document_transfer_number }}" data-toggle="tooltip" class="btn btn-danger btn-xs md-trigger cancel-action" data-original-title="{{ trans('shared/common.cancel') }}" data-modal="modal-cancel">
                                        <i class="fa fa-remove"></i>
                                    </a>
                                    @endif
                                    @if($model->status == DocumentTransferHeader::COMPLETE || $model->status == DocumentTransferHeader::INPROCESS)
                                    <a href="{{ URL($url . '/print-pdf-detail/' . $model->document_transfer_header_id) }}" data-toggle="tooltip" class="btn btn-xs btn-success" data-original-title="{{ trans('shared/common.print') }}" target="_blank">
                                        <i class="fa fa-print"></i>
                                    </a>
                                    @endif
                                    @if(Gate::check('access', [$resource, 'close']) && $model->status == DocumentTransferHeader::INPROCESS)
                                    <a data-id="{{ $model->document_transfer_header_id }}" data-label="{{ $model->document_transfer_number }}" data-toggle="tooltip" class="btn btn-danger btn-xs md-trigger close-action" data-original-title="{{ trans('shared/common.close') }}" data-modal="modal-close">
...
<div class="md-modal md-3d-flip-horizontal" id="modal-cancel">
    <div class="md-content">
        <h3 style="padding-bottom: 0px;"><strong>{{ trans('shared/common.cancel') }}</strong> {{ trans('operational/menu.document-transfer') }}</h3>
        <div>
            <div class="row">
                <div class="col-sm-12">
...
<script type="text/javascript">
    $(document).on('ready', function(){
        ...
        $('.cancel-action').on('click', function() {
            ...
            $("#cancel-text").html('{{ trans('purchasing/fields.cancel-confirmation', ['variable' => trans('operational/menu.document-transfer')]) }} ' + $(this).data('label') + '?');
            clearFormCancel()
        });
