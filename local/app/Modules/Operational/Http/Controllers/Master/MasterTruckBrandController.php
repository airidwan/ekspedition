...
class MasterTruckBrandController extends Controller
{
    const RESOURCE = 'Operational\Master\MasterTruckBrand';
    const URL      = 'operational/master/master-truck-brand';
    const DESC     = 'TRUCK';
    ...
    public function index(Request $request)
    {
        ...
        $query   = \DB::table('adm.mst_lookup_values')
                        ...
                        ->where('mst_lookup_values.lookup_type', '=', MasterLookupValues::MERK_KENDARAAN)
        ...
        return view('operational::master.master-truck-brand.index', [
    ...
    public function add(Request $request)
    {
        ...
        return view('operational::master.master-truck-brand.add', [
    ...
    public function save(Request $request)
    {
        ...
        $now    = new \DateTime();
        $model->lookup_type = MasterLookupValues::MERK_KENDARAAN;
        $model->meaning     = $request->get('brandName');
        $model->description = $request->get('description');
        $model->active      = $request->get('status') != 'Y' ? 'N' : 'Y';

        if (empty($id)) {
            $count                   = \DB::table('adm.mst_lookup_values')->where('lookup_type', '=', MasterLookupValues::MERK_KENDARAAN)->count();
            $model->lookup_code      = 'VCB.'.Penomoran::getStringNomor($count+1, 3);
        ...
        $request->session()->flash(
            'successMessage',
            trans('shared/common.saved-message', ['variable' => trans('operational/menu.truck-brand').' '.$model->lookup_code])
        );
